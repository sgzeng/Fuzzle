...
clang -g -c -emit-llvm $WORKDIR/file.c -o $WORKDIR/file.bc
...
$BEACON/Ins -output=$WORKDIR/beacon_file.bc -byte -blocks=$WORKDIR/bbreaches.txt -afl -log=log.txt -load=$WORKDIR/range_res.txt $WORKDIR/transed.bc
# 2.3.4 Compilation
clang $WORKDIR/beacon_file.bc -o $WORKDIR/file.bin_instrumented -lm -lz -ldl $BEACON/afl-llvm-rt.o
...
nohup timeout $TIMEOUT $BEACON/afl-fuzz -t 2000+ -m none -i $IN_DIR -o $OUT_DIR -d -- ${WORKDIR}/file.bin_instrumented > ${OUT_DIR}/beacon.log 2>&1 &
