...
touch $WORKDIR/.start
# fuzz
nohup timeout $TIMEOUT python3 $MAZERUNNER_SRC/mazerunner/mazerunner.py -a explore -m distance -i $IN_DIR -m reachability -o $OUT_DIR -s $AFLGO_TARGET_DIR -- $BUILD_DIR/${PROGRAM_NAME}_symsan_NM > ${OUT_DIR}/mazerunner.log 2>&1 &
